...
def _ensure_corpus(*, rebuild: bool) -> str:
    from retrieval import run_build_corpus
...
def _build_indexes(
    *,
    ...
    from retrieval import BM25PlusRetriever, BM25Retriever, VectorRetriever
...
def _handle_serve(args: argparse.Namespace) -> None:
    if args.target == "webui":
        ...
        module_name = "runners.experimentWebUI"
    else:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    for command, module_name, help_text in [
        ...
        ("build-term-mapping", "runners.buildTermMapping", "构建评测术语映射"),
        ("eval-retrieval", "modelEvaluation.evalRetrieval", "运行正式检索评测"),
        ("rag", "runners.runRag", "运行 RAG 问答"),
        ("experiments", "runners.runExperiments", "运行端到端对比实验"),
        ("eval-generation", "modelEvaluation.evalGeneration", "运行生成质量评测"),
        ("report", "runners.generateReport", "生成最终评测报告"),
    ]:
