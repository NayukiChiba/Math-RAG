...
from research.modelEvaluation.common.ioUtils import saveJsonFile
from research.modelEvaluation.common.paths import buildRetrievalAssets
...
def runEvalRetrieval(args: Any) -> int:
    ...
    if args.visualize and len(all_metrics) > 1:
        try:
            from reports_generation.viz.charting import generateComparisonChart

            output_dir = os.path.dirname(args.output)
            generateComparisonChart(all_metrics, output_dir)
        except ImportError:
            print("  跳过图表生成：reports_generation 未安装")
