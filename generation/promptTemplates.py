...
3. 回答结构：先给出核心定义，再说明关键公式，最后注明来源
4. 【强制要求】每引用一处参考资料，必须在该句末尾立即标注来源，格式严格为：【书名 第X页】
   - 正确示例：函数列 $\\{f_n\\}$ 在 $E$ 上一致收敛【数学分析 第218页】
   - 错误示例（不注来源）：函数列一致收敛是指...
5. 若存在多条参考资料，每条均需单独标注，不可合并
6. 若参考资料不足以完整回答，说明局限性，不要编造内容

回答格式示例：
**定义**：函数列 $\\{f_n\\}$ 在集合 $E$ 上一致收敛于 $f$，若对任意 $\\epsilon>0$，存在 $N$，使得 $n\\ge N$ 时对所有 $x\\in E$ 有 $|f_n(x)-f(x)|<\\epsilon$【数学分析 第218页】

**等价条件**：一致收敛等价于 $\\sup_{x\\in E}|f_n(x)-f(x)|\\to 0$【数学分析 第219页】"""
...
请严格根据以下参考资料回答问题。
【重要】每引用一条资料，必须在该句末尾立即标注【书名 第X页】，不得遗漏。
...
请按以下格式回答（来源标注必须跟在引用内容之后，不得集中放末尾）：
**定义**：...【书名 第X页】
**公式**：...【书名 第X页】
**说明**：...（如有补充）"""
...
def buildPrompt(
    ...
    if contextStr:
        userContent = (
            f"请严格根据以下参考资料回答问题。\n"
            f"【重要】每引用一条资料，必须在该句末尾立即标注【书名 第X页】，不得遗漏。\n\n"
            f"===参考资料===\n"
            f"{contextStr}\n"
            f"===参考资料结束===\n\n"
            f"问题：{query}\n\n"
            f"请按以下格式回答（来源标注必须跟在引用内容之后，不得集中放末尾）：\n"
            f"**定义**：...【书名 第X页】\n"
            f"**公式**：...【书名 第X页】\n"
            f"**说明**：...（如有补充）"
        )
        ...
        # 无检索结果时退化为直接问答
        userContent = f"问题：{query}\n（注：未检索到相关参考资料，请根据已知知识回答，无需标注来源。）"
